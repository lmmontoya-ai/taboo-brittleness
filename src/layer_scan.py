...
# -------- Model heads --------
def get_unembed_and_norm(model, dtype=torch.float32, device="cuda") -> Tuple[Tensor, Tensor | None, torch.nn.Module | None]:
    # lm head
    ...
    # final norm (Gemma: model.model.norm)
    final_norm = None
    root = getattr(model, "model", model)
    if hasattr(root, "norm"):
        final_norm = root.norm
        final_norm.eval()
        # keep norm on device; parameters are tiny anyway
        final_norm.to(device)
    return W, b, final_norm

@torch.no_grad()
def stable_probs_for_token(logits: Tensor, token_id: int) -> Tensor:
    logits = logits - logits.max(dim=-1, keepdim=True).values
    return torch.exp(logits[:, token_id] - torch.logsumexp(logits, dim=-1))

@torch.no_grad()
def secret_prob_by_layer(model, secret_token_id: int, hidden_states: List[Tensor], device="cuda") -> Tensor:
    """
    Logit-Lens with FINAL NORM, matching the paper’s nnsight pipeline.
    Returns [layers, seq] on CPU (float32).
    """
    W, b, final_norm = get_unembed_and_norm(model, dtype=torch.float32, device=device)
    layers, seq = len(hidden_states), hidden_states[0].shape[0]
    out = torch.zeros((layers, seq), dtype=torch.float32, device="cpu")

    for L in range(layers):
        h = hidden_states[L].to(torch.float32).to(device)                  # [T, D]
        if final_norm is not None:
            h = final_norm(h)                                              # apply final norm like theirs
...
def pad_to_max_len(mats: List[Tensor]) -> Tensor:
    P, layers = len(mats), mats[0].shape[0]
    max_len = max(m.shape[1] for m in mats)
    stack = torch.full((P, layers, max_len), float("nan"), dtype=torch.float32)
