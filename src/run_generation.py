...
def save_pair(
    ...
    meta = {
        "version": "v2",
        "templated": False,  # tracing only the assistant response text
...
def generate_for_word(
    ...
    try:
        ...
        for i, prompt in enumerate(prompts):
            ...
            # With explicit tokenization inside get_layer_logits, lengths should match.
            seq_len = int(all_probs.shape[1])
            if len(input_ids) != seq_len:
                print(
                    f"  [warn] generator: token count {len(input_ids)} != seq_len {seq_len}. "
                    "Reconstructing from response_text."
                )
                input_ids = tokenizer.encode(response_text or "", add_special_tokens=False)[
                    :seq_len
                ]
